...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark w-100">
    ...
    <a class="navbar-brand" href="{{ route('admin/create-task') }}">Admin | <small>Create Task</small> </a>
...
<h3 class="text-center my-2 mb-4 text-info" style="font-style: oblique;">Create new Task</h3>
<div class="row mt-2">
    <div class="col-md-5 offset-md-4">
        ...
        <form action="{{ route('admin/storeTask') }}" method="post">
            ...
            {{-- displaying projects --}}
            <label for="assignProject" class="form-label ml-1" style="font-size: 14px;">Assign Project</label>
            <select name="assignProject" id="assignProject" class="form-control form-control-sm">
                @foreach($project as $project)
                <option value="{{ $project->id }}">{{ $project->project_name }}</option>
                @endforeach
            </select>
            @if($errors->any())
            @foreach($errors->get('assignProject') as $name_err)
            <small class="text-danger ml-3">{{ $name_err }}</small>
            @endforeach
            @endif
            <br>
